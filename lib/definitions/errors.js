...
export function ESTAGECONFIG({ stage }) {
  return {
    message: `Stage ${stage} is not recognized`,
    details: "Use one of prepare, publish, notify",
  };
}

export function EPREVIOUSFAIL({ stage }) {
  return {
    message: `One of the previous stages failed, aborting`,
    details: "Examine the results from a previous stage",
  };
}

export function ENOSTATE({ stage, requiredStage }) {
  return {
    message: `Stage ${stage} requires ${requiredStage} to be completed.`,
    details: `Run all previous stages, such as \`semantic-release -s ${requiredStage}\` before this command. 

If you are using CI, make sure that the \`.semrel\` directory is passed between jobs`,
  };
}

export function EBADSTATE({ stage, requiredStage, previousStage }) {
  return {
    message: `Stage ${stage} must be run after ${requiredStage}, but was run after ${previousStage}.`,
    details: `Fix ordering of the \`semantic-release\` commands. If you need to start again, run \`semantic-release -s prepare\`.`,
  };
}
